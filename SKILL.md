...
- When a dense wrapper contains repeated preserved chairs, rails, shelves, table supports, or other structural children, classify each nested instance independently even when the wrapper bounds are fully visible. Keep clear children complete, discard fully covered children, and apply depth-aware edge clipping only to genuinely partial children.
