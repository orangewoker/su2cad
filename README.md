...
下载 GitHub Release 中的 `SU2CAD-0.5.0-windows-x64.zip`，完整解压后运行：
...
1. 共线线段合并。
2. 删除打印比例下不可见的微小线段。
3. 仅对植物等高密度网格块按二维网格均匀保留代表线；家具等普通块不套用植物线数上限。
...
8. 轻量模式对单个超大块采用 12000 个实体的集合采样上限和 20000 个实体的遍历预算；普通顶层组件不会被任意截断。
9. 俯视重复组件只计算一次真实块几何，其他实例以 `INSERT` 的旋转、镜像和比例复用，不生成虚构外包多边形。
10. 普通 SketchUp 硬边完整保留；仅柔化、平滑网格边继续按视向轮廓过滤。
...
当前大型办公模型的轻量模式实测约 1 分 55 秒，生成 1246 个块参照、94 个原生 CAD 圆，DXF 约 7.7 MB，审计错误为 0。实际结果取决于当前视口与模型复杂度。
