...
`dev` 分支包含不需要启动 Codex 的 Windows 桌面应用。v0.8.1 使用 Tauri 2 + React + TypeScript 和 WebView2 渲染响应式液态玻璃界面；原有 Python 几何导出核心作为内置 Sidecar 运行。桌面端可动态识别多个 SketchUp / AutoCAD 版本，并从顶部“插件”面板一键安装或修复连接组件。

下载 GitHub Release 中的 `SU2CAD_0.8.1_x64-setup.exe` 并安装。安装包已包含 Python、NumPy、Shapely、DXF 引擎以及 SketchUp / AutoCAD 插件载荷，用户不需要另行安装 Node.js、Rust 或 Python。

### 连接插件

- 桌面端：点击顶部“插件”，可查看检测到的 SketchUp / AutoCAD 版本并一键安装。
- SketchUp：可单独分享 `SU2CAD-SketchUp-Bridge-0.8.1.rbz`，从扩展程序管理器安装。
- AutoCAD：可单独分享 `SU2CAD-AutoCAD-Helper-0.8.1.zip`，解压 Bundle 到 `%APPDATA%\Autodesk\ApplicationPlugins\`。
- 合集：`SU2CAD-Plugins-0.8.1.zip` 包含自动扫描与安装脚本，详见 `plugins/README-安装说明.md`。
...
- 重复结构组件使用带深度的实例遮挡判定，不再让相邻桌面、地面或家具共用错误的屏幕深度缓存；同一椅子组件在不同位置保持一致的完整结构
- 外层复杂容器中的重复椅子、栏杆、桌腿和垫层会再次执行独立遮挡判定：完全可见才完整保留，完全遮挡直接跳过，真正部分遮挡的子组件才逐线裁剪
- 即使桌椅外层组整体处于可见状态，也会继续检查桌面内部的子组件，避免桌面下方看不到的支撑和垫层透出；当前测试模型块线总量由 231171 降至 94317，同时保留可见椅子结构
- 当前 740 万源实体测试模型的平衡模式完整导出与 DXF 审计耗时约 4 分 24 秒
- SketchUp 可见标签会继承为同名 `SU-*` CAD 图层；嵌套实体自己的标签优先于父组标签，并同步标签 RGB 颜色
- 超大集合不再只取最前面的实体，而是在整个集合中均匀取样，避免位于定义后部的家具主体或文字轮廓消失
- 平衡模式采用 100/145/200 秒三级提取预算：先完整处理重复结构和普通组件，再生成紧凑真实轮廓，最后以有界预算覆盖剩余高密度装饰；完整流程目标控制在 5 分钟内
...
desktop\src-tauri\target\release\bundle\nsis\SU2CAD_0.8.1_x64-setup.exe
```
