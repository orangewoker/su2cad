...
下载 GitHub Release 中的 `SU2CAD-0.3.0-windows-x64.zip`，完整解压后运行：
...
桌面应用将长时间的 SketchUp 提取拆成约 250 毫秒的时间片。每个时间片结束后，SketchUp 可以处理界面事件，应用也可以更新真实实体计数或响应取消，不再依赖单个 10 分钟 HTTP 请求。
