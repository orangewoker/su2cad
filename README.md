...
下载 GitHub Release 中的 `SU2CAD-0.7.2-windows-x64.zip`，完整解压后运行：
...
- 外层复杂容器中的重复椅子、栏杆和搁板会单独执行精细整组判定，完全遮挡才剔除；可见或仅部分遮挡的同组件复用一致几何，不再因外层容器被判为“部分遮挡”而随机缺线
- 当前测试模型的两个同类桌椅组由 2965/2957 条不一致线稿统一恢复为各 5718 条可编辑线稿；平衡模式完整导出与 DXF 审计耗时约 4 分钟
